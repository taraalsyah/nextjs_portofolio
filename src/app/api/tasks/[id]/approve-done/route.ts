...
export async function POST(
  ...
  try {
    ...
    if (userRole !== 'OWNER' && userRole !== 'ADMIN') {
      return NextResponse.json(
        { error: 'Hanya Owner atau Admin yang dapat menyetujui Done Request.' },
        { status: 403 }
      );
    }
    ...
    const updatedTask = await prisma.$transaction(async (tx) => {
      ...
      await logTaskActivity({
        ...
        action: 'DONE_APPROVED',
        description: `${reviewerName} approved the completion request.`,
        fieldName: 'status',
