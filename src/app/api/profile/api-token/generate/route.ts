...
import { generateUserApiToken } from '@/services/user/api-token.service';
...
export async function POST(request: Request) {
  ...
  try {
    const rawToken = await generateUserApiToken(userId, ipAddress, userAgent);
    ...
    return NextResponse.json(
      { success: false, message: 'Failed to generate API token.' },
      { status: 500 }
    );
  }
}
