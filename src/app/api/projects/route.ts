...
export async function POST(req: NextRequest) {
  try {
    ...
    const { projectName, description, visibility = 'PRIVATE' } = body;
    ...
    // Prisma transaction to create Project and ProjectMember as OWNER
    const project = await prisma.$transaction(async (tx) => {
      ...
      await tx.activityLog.create({
        data: {
          ...
          description: `Membuat Proyek Baru: "${created.projectName}"`,
        },
      });
