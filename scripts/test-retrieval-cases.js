// Direct JS verification script for vector service reranking & intent logic
...
function detectQueryIntent(query) {
    ...
    let targetEntity;
    if (qLower.includes("system web ticketing") || qLower.includes("ticketing")) {
    ...
    } else if (qLower.includes("smsc knowledge") || qLower.includes("smsc")) {
        targetEntity = "SMSC Knowledge";
    }
...
function rerankChunksWithDebug(query, candidates, topK = 5, customWeights) {
    ...
    for (const chunk of deduplicatedCandidates) {
        ...
        if (canonicalChildProjectExists && isParentContainer) {
            rejectedCandidates.push({
                id: chunk.id,
                document_id: chunk.document_id,
                section: sectionName,
    ...
    const scoredCandidates = hierarchyFilteredCandidates.map((chunk) => {
        ...
        if (primaryIntent === "project") {
            ...
            if (sectionLower.includes("skills") || sectionLower.includes("keahlian") || sectionLower.includes("kemampuan")) {
                ...
                sectionScore = 1.0;
                hierarchyScore = 1.0;
            } else if (sectionLower.includes("technical experiences") || sectionLower.includes("technical")) {
                intentScore = 0.8;
                sectionScore = 0.8;
                hierarchyScore = 0.8;
            } else {
        ...
        if (targetEntity && (sectionLower.includes(targetEntity.toLowerCase()) || contentLower.includes(targetEntity.toLowerCase()))) {
            exactMatchScore = 1.0;
            if (sectionLower.includes(targetEntity.toLowerCase())) {
